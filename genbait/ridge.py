...
def run_ridge(df_norm, k=None, seed=4):
    """
    Perform feature selection using Ridge and return the top-k selected baits.
    ...
    model = LogisticRegression(penalty='l2', solver='saga', random_state=46)
    model.fit(X_train, y_train)
    coef = model.coef_
    selected_idx = np.argsort(np.abs(coef).mean(axis=0))[::-1][:k]
