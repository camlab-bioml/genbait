...
def run_elastic_net(df_norm, k=None, seed=4):
    """
    Perform feature selection using ElasticNet and return the top-k selected baits.
    ...
    model = LogisticRegression(penalty='elasticnet', solver='saga', l1_ratio=0.5, random_state=46)
    model.fit(X_train, y_train)
    coefs = model.coef_
    selected_idx = np.argsort(np.abs(coefs).mean(axis=0))[::-1][:k]
